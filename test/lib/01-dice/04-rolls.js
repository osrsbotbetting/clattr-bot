  ...
  describe("wild", () => {
    it("should return a wild dice result object", () => {
      ...
      for (let i = 0; i < 10; ++i) {
        ...
        let wildResults = sumResults(
          roll.dice.filter(
            d =>
              d.marker !== pieceMarkers.WILD &&
              d.marker !== pieceMarkers.WILD_EXPLODED
          )
        );
        let normalResults = sumResults(
          roll.dice.filter(
            d =>
              d.marker === pieceMarkers.WILD ||
              d.marker === pieceMarkers.WILD_EXPLODED
        ...
        roll.result.should.eql(
          Math.max(wildResults, normalResults),
          `wild results: ${wildResults}, normalResults: ${normalResults}`
        );
      }
    });
    it("should default to one die of size 6", () => {
